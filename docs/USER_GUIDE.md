...
Your data stays in the browser on your device unless you export a backup. The app has no login, remote AI, analytics, advertisements, or tracking.
...
5. Open **Party Headquarters** whenever you want emotional support, practical help, the weekly Campfire, or a Monthly Council.
...
Each path can be completed once per System date. A second distinct path awards a **+150 account XP Double Deployment** surge, a third adds a **+200 account XP Triple Deployment** surge, and all four add a **+250 account XP Full Spectrum Deployment** surge, each with its displayed stat growth. Daily Workout itself still pays only once. Every tier is one-time, optional, and never multiplied by Snow’s Daily Command.
...
The companions' dedicated space. It contains:

- **Check-In:** Choose the mood that feels closest. Snow opens and closes while the whole party responds.
- **Direct Support:** Choose Motivation, Make a plan, Faith & perspective, Calm down, Recover, or Celebrate. Then choose the whole party or one specific companion.
- **Saved:** Revisit companion messages you marked with a heart.

Check-Ins and Direct Support are private, work offline, and never change XP, missions, streaks, rewards, or penalties.

### Treasury Command

Cassian's private money-management section. It works without a bank connection and contains:

- **Command:** Weekly Budget Stability, income, spending, future-directed money, and today's No Eating Out directive.
- **Ledger:** Manual paycheck, income, and expense entries, including a clear eating-out marker.
- **Bills:** Recurring obligations and payment records.
- **Debt:** Credit cards and other balances, APR notes, minimum payments, and actual payment records.
...
Perfect Days, completed challenges, Class Trials, Companion Quest chapters, Treasury reviews, the first three weekly Kitchen Orders, multi-path Training surges, and accepted rare events award their own displayed one-time XP. Campaign Arc milestones, Party conversations, Campfires, Councils, repeat Sanctuary studies, Stronghold sessions, ordinary Training Hall overtime, and cooking beyond the weekly Kitchen cap do not create hidden or repeatable XP. Snow’s Daily Command is the only broad positive multiplier, and it applies only to scheduled mission account and stat XP after Daily Review—not to Kitchen or Training surge rewards. These examples assume a Stable state: Warning temporarily awards 90% of printed account mission XP and Stagnant awards 75% until completed days restore stability; stat mission XP remains as printed. The command bonus is calculated from XP actually recorded, preventing hidden stacking or duplicate rewards.

## The companion party

- **Snow, The Constant:** Your primary whole-journey companion.
- **Rook, The Vanguard:** Strength, Endurance, and Vitality.
...
Amara owns Empathy stat reactions and relationship-focused Character mission responses. Her questline and support language always permit protected distance, private reflection, or reaching someone else trustworthy; the app never requires contact with an unsafe person.
...
## Rare events and Mission Passes

Each System day receives one saved rare-event roll. Refreshing the app cannot reroll it.

- An optional **Emergency Quest** can be accepted or declined without penalty.
- A **Mission Pass** can protect one pending required mission without using a monthly protected exception. It does not award that mission's XP.
- Most days have no rare event.

## Hard days, missed days, and recovery

A missed mission or broken streak remains honestly visible, but it does not erase levels or permanently lock the journey.

Useful options include:

- Open **Party Channel → Direct Support → Recover**.
- Activate **Recovery Mode** in Settings.
- Excuse a mission when the situation genuinely qualifies.
- Use a saved Mission Pass.
- Reduce the day to one realistic action and return tomorrow.

The app is not medical care, crisis support, pastoral care, or a substitute for trusted people. If you feel unsafe or need human help, contact someone you trust or an appropriate local support service.

## Appearance and phone use

The System interface is the more game-like presentation. Clean reduces decorative effects. Abyss is the dark theme; Daybreak uses a light gray, navy, and sun-yellow palette. Visual intensity and reduced motion can be adjusted separately.

For the intended full-screen phone experience, open the GitHub Pages address in Safari and choose **Share → Add to Home Screen**.

## Backups and changing phones

Your progress is not stored in GitHub. It lives in the browser data on the current device.

Before clearing browser data, changing phones, or installing a major release:

1. Open **Update Center → Archive Shield** or **Settings → Archive Shield · Local Data**.
2. Choose **Export full save**.
...
After a new version is pushed to GitHub and the Pages workflow succeeds, open the installed app while online. Accept the update prompt when it appears, or open **Update Center** and tap **Check for update**. Existing data is migrated locally rather than replaced. You do not need to delete the home-screen app.

Exporting a backup before a major update is still the safest habit.
